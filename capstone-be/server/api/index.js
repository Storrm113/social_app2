...
export const fetchCommunityPosts = async () => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/communitiespost/`);
    return response.data;
  } catch (error) {
...
export const registerUser = async (userData) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/api/users/register`, userData, { timeout: 10000 }); // 10s timeout
    return response.data;
  } catch (error) {
